...
struct
  ...
  let is_bot _ = false
  let isSimple _  = true
  let short _ (x,t) = string_of_float x
  let pretty_f _ _ (x,t) = text (string_of_float x)
  let toXML_f _ (x,t) = Xml.Element ("Leaf", [("text", string_of_float x)],[])
  let toXML m = toXML_f short m
  let pretty () x = pretty_f short () x
  let leq (x,t1) (y,t2) = x <= y
  let pretty_diff () ((x,t1),(y,t2)) = Pretty.dprintf "%a instead of %a" pretty x pretty y
  let join (x,t1) (y,t2) = if compare x y < 0 then (y,t2) else (x,t1)
  let meet (x,t1) (y,t2) = if compare x y > 0 then (y,t2) else (x,t1)

  let of_bool (x,t) = if x then (1.0,t) else (0.0,t)
  let to_bool' x = x <> 0.0
  let to_bool (x,t) = Some (to_bool' x)
  let is_bool _ = true
  let of_float (x,t) = (x,t)
  let to_float (x,t) = Some x
  let is_float  _ = true
  ...
  let l1 f (x,t) = (f x, t)
  let l2 f (x1,t1) (x2,t2) = (f x1 x2, super t1 t2)
...
(*  let div x y = (* nan instead? *)
  ...
  let rem  = l2 mod_float
  let cmp f (n1,t1) (n2,t2) = of_bool ((f n1 n2), super t1 t2)
  let lt = cmp (<)
  let gt = cmp (>)
  let le = cmp (<=)
  let ge = cmp (>=)
  let eq = (*let _ = printf "float:eq: %f = %f -> %B\n" n1 n2 (n1=n2) in *) cmp (=)
  let ne = cmp (<>)
  let bit' f (x,t1) (y,t2) = (Int64.float_of_bits (f (Int64.bits_of_float x) (Int64.bits_of_float y)), super t1 t2)
  let bitnot (x,t) = (Int64.float_of_bits (Int64.lognot (Int64.bits_of_float x)), t)
  let bitand = bit' Int64.logand
  let bitor  = bit' Int64.logor
  let bitxor = bit' Int64.logxor
  let shift_left  (n1,t1) (n2,t2) = (Int64.float_of_bits (Int64.shift_left (Int64.bits_of_float n1) (int_of_float n2)), super t1 t2)
  let shift_right (n1,t1) (n2,t2) = (Int64.float_of_bits (Int64.shift_right (Int64.bits_of_float n1) (int_of_float n2)), super t1 t2)
  let lognot (n1,t1)    = of_bool (not (to_bool' n1), t1)
  let logand (n1,t1) (n2,t2) = of_bool ((to_bool' n1) && (to_bool' n2), super t1 t2)
  let logor  (n1,t1) (n2,t2) = of_bool ((to_bool' n1) || (to_bool' n2), super t1 t2)
  let logxor (n1,t1) (n2,t2) = let a = (to_bool' n1) in let b = (to_bool' n2) in of_bool (a && (not b) || (not a) && b, super t1 t2)
...
(*  let round_downward (up, down) xb restZero =
  ...
  let doubleToFloatDomain x =
    match Base.to_float x with
      | Some x -> Base.of_float (doubleToFloat x, `Float)
      | _ -> x
  ...
  let cilToFD = function Cil.FLongDouble -> `LongDouble | Cil.FDouble -> `Double | _ -> `Float
  let of_int (x,t) = match x with Some x -> Base.of_float (Int64.to_float x, cilToFD t) | None -> raise Unknown
end
...
struct
  ...
  let to_bool x = 
    match x with
      ...
      | `Excluded s when S.mem (0.0, `Float) s -> Some true
      | _ -> None
  let is_bool x = 
    match x with
      | `Definite x -> true
      | `Excluded s -> S.mem (0.0, `Float) s
      | _ -> false
  ...
  let eq x y = (*print_string "float:Trier:eq:";*) match x,y with
     ...
     * don't know: *)
    | `Definite (x,t), `Excluded s -> if S.mem (x,t) s then of_bool (false,t) else top ()
    | `Excluded s, `Definite (x,t) -> if S.mem (x,t) s then of_bool (false,t) else top ()
    (* The good case: *)
    | `Definite (x,t1), `Definite (y,t2) -> (*let _ = printf "`Definite %f, `Definite %f -> %B\n" x y (x=y) in*) of_bool (x=y, super t1 t2)
  ...
  let ne x y = match x,y with
     ...
     * don't know: *)
    | `Definite (x,t), `Excluded s -> if S.mem (x,t) s then of_bool (true,t) else top ()
    | `Excluded s, `Definite (x,t) -> if S.mem (x,t) s then of_bool (true,t) else top ()
    (* The good case: *)
    | `Definite (x,t1), `Definite (y,t2) -> of_bool (x<>y, super t1 t2)
    (* If either one of them is bottom, we return bottom *)
    | _ -> `Bot
...
(*  let lognot = eq (of_int 0.0) *)
  let lognot = eq (of_float (0.0, `Float)) (* ME *)
end


module InfFloat =
struct
  type t = NInf | Fin of Floats.t | PInf
  ...
  let addp d x y = 
    match x, y with
      ...
      | NInf , PInf  -> d
      | Fin _, NInf  -> NInf
      | Fin x, Fin y -> Fin (Floats.add x y)
      | Fin x, PInf  -> PInf
      | PInf , NInf  -> d
      | PInf , Fin x -> PInf
      | PInf , PInf  -> PInf
  ...
  let mul x y = 
    let nf (x,t) = match Pervasives.compare x 0.0 with 
      | 0          -> Fin (0.0, t)
      | x when x<0 -> PInf 
      | _          -> NInf
    in
    let pf (x,t) = match Pervasives.compare x 0.0 with 
      | 0          -> Fin (0.0, t)
      | x when x<0 -> NInf 
      | _          -> PInf
    in
    match x, y with
      ...
      | NInf , PInf  -> NInf
      | Fin x, NInf  -> nf x 
      | Fin x, Fin y -> Fin (Floats.mul x y)
      | Fin x, PInf  -> pf x
      | PInf , NInf  -> NInf
      | PInf , Fin x -> pf x
      | PInf , PInf  -> PInf
  ...
  let divp d x y = 
    ...
    match x, y with
      ...
      | NInf , PInf  -> d
      | Fin x, NInf  -> nf x 
      | Fin x, Fin y -> Fin (Floats.div x y)
      | Fin x, PInf  -> pf x
      | PInf , NInf  -> d
      | PInf , Fin x -> pf x
      | PInf , PInf  -> d
...
struct 
  ...
  let ending   x = (I.NInf , I.Fin (x, `Float))
  let starting x = (I.Fin (x, `Float), I.PInf)
  let maximal (_,y:t) = 
    match y with
      | I.Fin x -> Floats.to_float x
      | _ -> None
  let minimal (x,_:t) = 
    match x with
      | I.Fin x -> Floats.to_float x
  ...
  let to_float (x,y:t) = 
    match x, y with
      | I.Fin (x,t1), I.Fin (y,t2) when Pervasives.compare x y == 0 -> Floats.to_float (x,t1)
      | _ -> None
  let is_float (x,y:t) = 
    match x, y with
      | I.Fin (x,t1), I.Fin (y,t2) when Pervasives.compare x y == 0 -> true
      | _ -> false

  let of_bool (b,t) = if b then (I.Fin (1.0, t), I.PInf) else (I.Fin (0.0 ,t), I.Fin (0.0, t))
  let to_bool i = 
    match i with
      | I.Fin (0.0, _), I.Fin (0.0, _) -> Some false
      | _ when not (leq (of_float (0.0, `Float)) i) -> Some true
      | _ -> None
  let is_bool i =
  ...
  let rec div (x1,x2:t) (y1,y2:t) =
    if is_bot (x1, x2) || is_bot (y1, y2) then
      ...
      match y1, y2 with
        | I.Fin (0.0, `Float), I.Fin (0.0, `Float) -> bot ()
        | I.Fin (0.0, `Float), _        -> div (x1,x2) (I.Fin (1.0, `Float),y2)
        | _      , I.Fin (0.0, `Float)  -> div (x1,x2) (y1, I.Fin (-1.0, `Float))
        | _ when leq (of_float (0.0, `Float)) (y1,y2) -> top ()
        | _ -> 
  ...
  let log f i1 i2 = 
    ...
    match to_bool i1, to_bool i2 with
      | Some x, Some y -> of_bool (f x y, `Float)
      | _              -> top ()
  ...
  match to_float i1, to_float i2 with
    | Some x, Some y -> of_float (Int64.to_float (f (Int64.of_float x) (Int64.of_float y)), `Float)
    | _              -> top ()
  ...
  let bit1 f i1 = 
    ...
    match to_float i1 with
      | Some x -> of_float (Int64.to_float (f (Int64.of_float x)), `Float)
      | _      -> top ()

  let bitnot = bit1 Int64.lognot
  let shift_right _ _ = bot ()
  let shift_left _ _  = bot ()
  ...
  let ne i1 i2 =
    ...
    match sub i1 i2 with
      | (I.Fin (0.0, t1), I.Fin (0.0, t2)) -> of_bool (false, super t1 t2)
      | x when not (leq (I.Fin (0.0, `Float), I.Fin (0.0, `Float)) x) -> of_bool (true, `Float)
      | _ -> top () 
  ...
  let eq i1 i2 = 
    match is_bot i1, is_bot i2 with
      ...
      | _ ->
    match sub i1 i2 with
      | (I.Fin (0.0, t1), I.Fin (0.0, t2)) -> (*print_endline "true";*) of_bool (true, super t1 t2)
      | x when not (leq (I.Fin (0.0, `Float), I.Fin (0.0, `Float)) x) -> of_bool (false, `Float)
      | _ -> top () 
  ...
  let ge (x1,x2) (y1,y2) =
    if is_bot (x1, x2) || is_bot (y1, y2) then
      ...
      if I.leq y2 x1 then of_bool (true, `Float)  else 
      if I.lt  x2 y1 then of_bool (false, `Float) else
      top ()
    end
  ...
  let le (x1,x2) (y1,y2) =
    if is_bot (x1, x2) || is_bot (y1, y2) then
      ...
      if I.leq x2 y1 then of_bool (true,`Float)  else
      if I.lt  y2 x1 then of_bool (false, `Float) else 
      top ()
    end
  ...
  let gt (x1,x2) (y1,y2) =
    if is_bot (x1, x2) || is_bot (y1, y2) then
      ...
      if I.lt  y2 x1 then of_bool (true, `Float)  else 
      if I.leq x2 y1 then of_bool (false, `Float) else
      top ()
    end
  ...
  let lt (x1,x2) (y1,y2) =
    if is_bot (x1, x2) || is_bot (y1, y2) then
      ...
      if I.lt  x2 y1 then of_bool (true, `Float)  else
      if I.leq y2 x1 then of_bool (false, `Float) else 
      top ()
    end
...
(*    | `Bot, _    -> true
  ...
  let norm (x:t) : t = 
    match x with
      ...
      | `Excluded (InfFloat.Fin (x,t),InfFloat.PInf ) -> `Included (InfFloat.NInf, InfFloat.Fin ((-.) x 1.0, t))
      | `Excluded (InfFloat.NInf ,InfFloat.Fin (x,t)) -> `Included (InfFloat.Fin ((+.) x 1.0, t), InfFloat.PInf)
      | x -> x
...
(*      |    x, `Bot 
      ...
      | `Excluded (x1, x2), `Included (y1, y2) -> 
        if InfFloat.lt y2 x1 || InfFloat.lt x2 y1 then
          ...
          norm (`Excluded (InfFloat.addp InfFloat.PInf y2 (InfFloat.Fin (1.0, `Float)) , x2))
        else if InfFloat.leq x2 y2 then
          norm (`Excluded (x1, InfFloat.addp InfFloat.NInf y2 (InfFloat.Fin (-1.0, `Float))))
        else 
          top ()
...
(*      |    _, `Bot 
      ...
      | `Excluded (x1, x2), `Included (y1, y2) -> 
        if InfFloat.lt y2 x1 || InfFloat.lt x2 y1 then
          ...
          `Included (InfFloat.addp InfFloat.NInf x2 (InfFloat.Fin (1.0, `Float)),y2)
        else if InfFloat.leq y2 x2 then
          `Included (y1, InfFloat.addp InfFloat.NInf x1 (InfFloat.Fin (-1.0, `Float)))
        else
...
(*   let of_interval x y : t = `Included (I.of_interval x y) *)
  ...
  let to_excl_list (x:t)  = 
    ...
    match x with 
      | `Excluded (InfFloat.Fin (x,t1), InfFloat.Fin (y,t2)) -> Some (els x y)  
      | _ -> None
  ...
  let of_excl_list x = 
    match x with  
      ...
      | x::xs ->
        ...
        `Excluded (InfFloat.Fin (x, `Float), InfFloat.Fin (y, `Float))
  ...
  let to_bool (x:t) =
    match x with
      ...
      | `Excluded x when I.leq (I.of_float (0.0, `Float)) x -> Some true
      | _ -> None

  let is_bool (x:t) =
    match x with
      | `Included x -> I.is_bool x
      | `Excluded x -> I.leq (I.of_float (0.0, `Float)) x

  let of_bool (x,t) : t = 
    match x with
      | true  -> `Excluded (I.of_float (0.0, t))
      | false -> `Included (I.of_float (0.0, t))
  ...
  let bitnot x : t =
    ...
    match to_float x with
      | Some x -> of_float (Floats.lognot (x, `Float))
      | _ -> top ()
  ...
  let eq (x:t) (y:t) : t =     
    let eq i e = if I.leq i e then of_bool (false, `Float) else top () in
    match x, y with
      | `Included x, `Included y 
        -> begin match I.to_bool (I.eq x y) with 
            | Some x -> of_bool (x, `Float)
            | None -> top () end
      | `Excluded x, `Excluded y -> top ()
  ...
  let ne (x:t) (y:t) : t =     
    let ne i e = if I.leq i e then of_bool (true, `Float) else top () in
    match x, y with
      | `Included x, `Included y 
